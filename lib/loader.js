...
var loadQuiverComponentsFromPath = function(path, callback) {
  requireAsync(path, function(err, module) {
    if(err) return loadComponentsFromDirectory(path, callback)

    if(module.quiverModule) {
      module.quiverModule(function(err, quiverModule) {
        if(err) return callback(err)

        callback(null, quiverModule.quiverComponents)
      })
    } else if(module.quiverComponents) {
      callback(null, module.quiverComponents)
    } else {
      callback(null, error(500, 'module ' + path + 
        ' contain neither quiver module nor component'))
    }
  })
}

module.exports = {
  loadQuiverComponentsFromPath: loadQuiverComponentsFromPath,
