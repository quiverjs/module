...
var loadComponentsFromPathSync = function(path) {
  var fileStats = fs.statSync(path)

  if(fileStats.isDirectory() && 
    !fs.existsSync(pathLib.join(path, 'package.json')) && 
    !fs.existsSync(pathLib.join(path, 'index.js'))) 
  {
    return loadComponentsFromDirectorySync(path)
  }

  var module = require(path)
  if(module.quiverModule) {
    return loadComponentsFromQuiverModule(module.quiverModule)
...
var exportFromManifestSync = function(nodeRequire, manifestPath) {
  ...
  var quiverComponents = loadComponentsFromPathSync(componentPath)
...
module.exports = {
  ...
  loadComponentsFromPathSync: loadComponentsFromPathSync,
